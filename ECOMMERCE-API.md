...
```bash
# Create a store
curl -X POST http://127.0.0.1:80/api/v2/stores \
  -H "Content-Type: application/json" \
  -d '{"user_id": 1, "name": "My Store", "description": "Test store"}'

# Get user's stores  
curl http://127.0.0.1:80/api/v2/users/1/stores
```

3. **Register and login**:
```bash
# Register as seller
curl -X POST http://127.0.0.1:80/api/v2/register \
  -H "Content-Type: application/json" \
  -d '{"username": "seller1", "email": "seller@test.com", "password": "password123", "role": "seller", "first_name": "John", "last_name": "Doe"}'

# Login to get token
curl -X POST http://127.0.0.1:80/auth/login \
  -H "Content-Type: application/json" \
...
curl -X POST http://127.0.0.1:80/api/v2/stores/1/products \
