...
export default function MoneyOutPage() {
  ...
  const [stats, setStats] = useState({ totalSpent: 0, vatReclaimable: 0, entryCount: 0 });
  ...
  const columns: GridColumn[] = [
    ...
    { key: 'supplier', header: 'Supplier', width: '180px', type: 'text', placeholder: 'Supplier name' },
    {
      key: 'category',
      header: 'Category',
      width: '200px',
    ...
    { key: 'amount', header: 'Amount Out', width: '130px', type: 'currency', required: true, placeholder: '0.00' },
    {
      key: 'vatRate',
      header: 'VAT Rate',
      width: '150px',
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const expenseRows: GridRow[] = transactions
      ...
      const vatReclaimable = expenseRows.reduce(
        (sum: number, r: GridRow) => sum + (parseFloat(r.data.vatAmount) || 0),
        0
      );
      setStats({ totalSpent, vatReclaimable, entryCount: expenseRows.length });
    } catch {
    ...
    (rowIndex: number, field: string, value: string) => {
      setRows((prev) => {
        ...
        if (field === 'amount' || field === 'vatRate') {
          row.data.vatAmount = computeVat(row.data.amount, row.data.vatRate);
        }
    ...
    async (rowIndex: number) => {
      ...
      try {
        ...
        const txDescription = [
          ...
          row.data.supplier ? `(${row.data.supplier})` : '',
        ...
        if (row.data.category) {
          ...
          await accountingService.createJournalEntry({
            ...
            description: `Purchase: ${row.data.description}`,
        ...
        setStats((prev) => ({
          ...
          vatReclaimable: prev.vatReclaimable + vatAmount,
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg sm:text-2xl font-bold text-secondary-900 mt-1 tabular-nums">
                {formatCurrency(stats.totalSpent, 'GBP', 'en-GB')}
              </p>
            </div>
