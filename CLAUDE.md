...
**Consumer products (external repos) DO NOT add migrations here.** OpsAPI is a shared SaaS platform; consumer products (e.g. `diy-tax-return-uk`) ship their schema via the plugin system (see "Pluggable projects" below), tracked in the separate `project_migrations` table. If you're adding a table for a specific consumer product, close this PR and open one in that consumer's repo instead.
...
### Pluggable projects
`projects/<name>/` (e.g. `hospital-patient-manager`) is a self-contained module with its own `project.lua` manifest, `api/`, `migrations/`, `services/`, `dashboards/`, and `themes/`. `helper/project-loader.lua` auto-discovers them at boot (from `OPSAPI_PROJECTS_DIR`, default `/app/projects`) and registers their routes + features dynamically via `ProjectConfig.registerFeature`. Migrations under `projects/<name>/migrations/*.lua` are run by `helper/project-migrator.lua` (each returns `function(schema, db)`) and tracked per-product in `project_migrations(project_code, migration_name, checksum, UNIQUE(project_code, migration_name))` — separate from opsapi's core `lapis_migrations`.

**External consumer products** (repos that use opsapi as a SaaS backend, e.g. `diy-tax-return-uk`) plug into the SAME system without living in this repo: they ship a tiny image `FROM bwalia/opsapi:<tag>` that `COPY`s their own `migrations/` + `project.lua` into `/app/projects/<code>/`, then runs `lapis exec "require('helper.project-migrator').migrate(code, path)"` as a k8s pre-upgrade hook. From opsapi's side this is invisible — the runner does not care whether a project was baked into the image or overlaid at deploy time. **Do not add consumer-specific migrations to `lapis/migrations/`; direct contributors to their own product repo.**
