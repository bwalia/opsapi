...
function HospitalStaffModel:search(criteria)
    ...
    local query =
    "SELECT hs.*, u.first_name, u.last_name, u.email as user_email FROM hospital_staff hs LEFT JOIN users u ON hs.user_id = u.id " ..
    where_clause .. " ORDER BY hs.department ASC, hs.position ASC"
...
function HospitalStaffModel:getWithUserDetails(staff_id)
    ...
    "SELECT hs.*, u.first_name, u.last_name, u.email as user_email FROM hospital_staff hs LEFT JOIN users u ON hs.user_id = u.id WHERE hs.id = ?",
        staff_id)
...
function HospitalStaffModel:getStatistics(hospital_id)
    ...
    -- Active staff
    local active_staff = db.select(
    "SELECT COUNT(*) as count FROM hospital_staff WHERE hospital_id = ? AND status = 'active'", hospital_id)
    stats.active_staff = active_staff[1] and active_staff[1].count or 0

    -- Staff by department
    local staff_by_dept = db.select(
    "SELECT department, COUNT(*) as count FROM hospital_staff WHERE hospital_id = ? GROUP BY department", hospital_id)
    stats.by_department = {}
    ...
    -- Staff by position
    local staff_by_position = db.select(
    "SELECT position, COUNT(*) as count FROM hospital_staff WHERE hospital_id = ? GROUP BY position", hospital_id)
    stats.by_position = {}
    ...
    -- Staff by shift
    local staff_by_shift = db.select(
    "SELECT shift, COUNT(*) as count FROM hospital_staff WHERE hospital_id = ? GROUP BY shift", hospital_id)
    stats.by_shift = {}
