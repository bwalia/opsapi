...
function PatientDocumentModel:getByType(patient_id, document_type)
    return self:select("WHERE patient_id = ? AND document_type = ? ORDER BY document_date DESC", patient_id,
        document_type)
end
...
function PatientDocumentModel:getStatistics(patient_id)
    ...
    -- Documents by type
    local documents_by_type = db.select(
        "SELECT document_type, COUNT(*) as count FROM patient_documents WHERE patient_id = ? GROUP BY document_type",
        patient_id)
    stats.by_type = {}
    ...
    local confidential_docs = db.select(
        "SELECT COUNT(*) as count FROM patient_documents WHERE patient_id = ? AND is_confidential = true", patient_id)
    stats.confidential_documents = confidential_docs[1] and confidential_docs[1].count or 0

    -- Recent documents (last 30 days)
    local recent_documents = db.select(
        "SELECT COUNT(*) as count FROM patient_documents WHERE patient_id = ? AND created_at >= CURRENT_DATE - INTERVAL '30 days'",
        patient_id)
