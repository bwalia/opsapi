--[[
    Academy Menu Items Seeding Migration

    Academy has backend (routes/academy.lua), DB tables (academy_courses /
    academy_lessons), an RBAC module ("courses") and a dashboard page
    (/dashboard/academy). This seeds the sidebar menu_items row, registers the
    RBAC module, grants permissions to owner/admin namespace roles, and enables
    the menu item for existing namespaces. Mirrors timesheet-menu-items.
    Feature-gated under FEATURES.ACADEMY.
]]

local db = require("lapis.db")

return {
    -- [1] Insert the Academy menu item
    [1] = function()
        ...
        for _, item in ipairs(items) do
            ...
            if #existing == 0 then
                ...
                print("[Academy] Added menu item: " .. item.key)
            end
        end
    end,

    -- [2] Register the academy RBAC module ("courses")
    [2] = function()
        local MigrationUtils = require("helper.migration-utils")
        local timestamp = MigrationUtils.getCurrentTimestamp()

        local modules = {
            { machine_name = "courses", name = "Courses", description = "Course and lesson content management (rich WYSIWYG)", priority = 44 },
        }
        ...
        for _, mod in ipairs(modules) do
            ...
            if #existing == 0 then
                ...
                print("[Academy] Registered module: " .. mod.machine_name)
            end
        end
    end,

    -- [3] Grant "courses" permissions to owner + admin roles
    [3] = function()
        local cjson_ok, cjson = pcall(require, "cjson")
        if not cjson_ok then return end

        local full_modules = { "courses" }
    ...
    [4] = function()
        ...
        for _, key in ipairs({ "academy" }) do
