--[[
    Field Service — Service Requests menu + RBAC seeding

    Surfaces the complaint queue in the backend-driven sidebar and registers its
    RBAC module:
    - "Service Requests" (/dashboard/field-service/requests) → module fs_service_requests
...
local MODULES = {
    { machine_name = "fs_service_requests", name = "Service Requests", description = "Customer complaints and service requests (the intake queue)", priority = 37 },
}

local MENU_KEYS = { "field_service_requests" }
...
return {
    ...
    [1] = function()
        ...
        local items = {
            {
                key = "field_service_requests",
                name = "Service Requests",
                icon = "ClipboardList",
                path = "/dashboard/field-service/requests",
                module = "fs_service_requests",
