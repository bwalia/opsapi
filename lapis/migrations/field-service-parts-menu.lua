--[[
    Field Service — Parts catalog menu + RBAC seeding

    - "Parts" (/dashboard/field-service/parts) → module fs_parts
...
local MODULES = {
    { machine_name = "fs_parts", name = "Parts", description = "Parts / products catalog for field service", priority = 42 },
}

local MENU_KEYS = { "field_service_parts" }
...
return {
    ...
    [1] = function()
        ...
        local items = {
            {
                key = "field_service_parts",
                name = "Parts",
                icon = "Boxes",
                path = "/dashboard/field-service/parts",
                module = "fs_parts",
